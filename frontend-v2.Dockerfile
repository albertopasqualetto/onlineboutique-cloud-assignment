...
# modified from the original to add new v2 endpoint
...
RUN sed -i 's~NewRouter()~NewRouter()\n\tr.HandleFunc(baseUrl + "/v2.txt", func(w http.ResponseWriter, _ *http.Request) { fmt.Fprint(w, "THIS IS VERSION v2") })~' main.go
